...
-- if we run the above we will see records by continent and we only want records based on country so we need to remove those for only continents
SELECT location,
		MAX(cast(total_deaths AS int)) AS HighestDeathCount  --cast function changes the data type from varchar to int
FROM Covid19.dbo.CovidDeaths
WHERE continent IS NOT NULL -- filters for only countries
GROUP BY location
order by HighestDeathCount desc
